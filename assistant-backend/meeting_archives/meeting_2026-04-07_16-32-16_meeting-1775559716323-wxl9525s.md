# Meeting Archive: meeting-1775559716323-wxl9525s
**Date:** April 07, 2026 at 04:32 PM
...
### assistant
### 1. The True Intent  
They want to know how to handle hallucination spikes during a shadow rollout and adjust the pipeline without moving to a more expensive language model.

### 2. The Current Pivot & Cheat Sheet  
**They want to know how to handle hallucination spikes during shadow rollout and adjust the pipeline without moving to a more expensive L.**  
- I’d add a real‑time safety filter that flags hallucinations and routes those requests back to the legacy model.  
- I’d throttle the new model’s traffic until the hallucination rate falls below a set threshold.  
- I’d implement a lightweight post‑processor that cleans up outputs before they reach the user, keeping costs low.

### 3. Architect Follow-Ups  
1. What is the current threshold for acceptable hallucination rate, and how is it measured?  
2. Do you have a real‑time monitoring system that can trigger an automatic rollback or traffic throttling based on hallucination spikes?
...
### assistant
### 1. Core Strategy  
I'd start with a shadow rollout, sending a small slice of traffic to the new model while keeping the legacy model live.  
If hallucination spikes, I’d route those requests back to the legacy model in real time and throttle the new model’s traffic until the spike subsides.  
I’d add a lightweight post‑processor that cleans up outputs before they reach the user, keeping costs low.  
Success is measured by a stable hallucination rate, low latency, and continued user engagement as the traffic split gradually increases.  

### 2. Explicit Metrics (The Telemetry)  
* **Hallucination Rate:** the percentage of responses flagged as hallucinations per 1,000 requests.  
* **Latency per Request:** average inference time for each model call.  
* **Safety Flag Rate:** number of outputs flagged by the content filter per 1,000 requests.  
* **Rollback Trigger Count:** how many times the automated safety guard kicked in during the rollout.  

### 3. Implicit Metrics (User Behavior)  
* **Session Length:** average time users spend interacting with the AI feature.  
* **Repeat Prompt Rate:** how often users re‑ask the same question within a session.  
* **Feature Adoption Rate:** percentage of users who continue to use the AI feature after the rollout.  

### 4. Edge Cases & Risks  
One risk is that the new model’s hallucination spike could be caused by a narrow data distribution, leading to a false sense of safety if we only throttle traffic.  
Another pitfall is that the rollback logic might be too aggressive, stalling adoption even when the hallucination rate is acceptable.  
Finally, if the post‑processor is too heavy, it could introduce latency that drives users back to the legacy model.
...
### assistant
### 1. The True Intent  
They want a strategy to handle hallucination spikes during a shadow rollout and adjust the pipeline without moving to a more expensive model.

### 2. The Current Pivot & Cheat Sheet  
**They want a strategy to handle hallucination spikes during a shadow rollout and adjust the pipeline without moving to a more expensive model.**  
- Use a real‑time safety filter to flag hallucinations and route those requests back to the legacy model.  
- Throttle the new model’s traffic until the hallucination rate falls below a set threshold.  
- Add a lightweight post‑processor that cleans up outputs before they reach the user, keeping costs low.

### 3. Architect Follow-Ups  
1. What is the acceptable hallucination rate threshold for triggering throttling or rollback?  
2. Do you have a real‑time monitoring system that can automatically trigger rollback or traffic throttling based on hallucination spikes?
...
### assistant
If hallucinations spike, route flagged responses back to the legacy model and throttle the new model's traffic.  
This keeps cost low while you keep the new model in play.  
Add a lightweight post‑processor to clean outputs before they hit users.  
Set a threshold for hallucination rate; auto‑rollback if exceeded.  
That way you avoid expensive model swaps and keep latency stable.

### assistant
Could not transcribe audio. Try again.

### user
🎧 **[Live System Audio]:**  We want to deploy a rock pipeline. similar to the one you built, using a zur open eye. How do you plan a shadow rollout for an AI feature? And what exact telemetry do you look at to ensure it is
